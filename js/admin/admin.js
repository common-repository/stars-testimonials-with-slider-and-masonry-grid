...
jQuery( document ).ready( function(){
    ...
    jQuery('.read-more-color-picker').ColorPicker({
        livePreview:false,
        color:"string",
        onSubmit: function(hsb, hex, rgb, el) {
            jQuery(el).val(hex);
            jQuery("#read-more").css("background-color", "#"+jQuery(el).val());
        },
        onChange: function(hsb, hex, rgb) {
            selectedColor = "rgb("+rgb.r+","+rgb.g+","+rgb.b+")";
            colorCode = rgb2hex(selectedColor);
            $m = jQuery('.read-more-color-picker');
            $m.val(colorCode);
            jQuery("#read-more").css("background-color", "#"+colorCode);
        },
        onBeforeShow: function () {
            jQuery(this).ColorPickerSetColor(this.value);
        }
    });
    jQuery(document).on("click", "#has_read_more", function(){
        if(jQuery(this).is(":checked")) {
            jQuery(".has-read-more").addClass("active");
        } else {
            jQuery(".has-read-more").removeClass("active");
        }
    });
